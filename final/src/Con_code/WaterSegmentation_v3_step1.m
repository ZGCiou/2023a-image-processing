...
%% pre-processong
dataPath = srcPath;
resPath = '../result/pre_demo/';
mkdir(resPath);
parfor i = 1:imgNum
    img = imread([dataPath, 'input', num2str(i), postfix]);
...
parfor i = 1:imgNum
    img = imread([dataPath, int2str(i), postfix]);
    imgCom = imresize(img, 0.5);
    [imgMS, clusterNum(i)] = Ms(imgCom,bw);
    imgMS = imresize(imgMS, size(img, [1 2]));
...
dataPath = resPath;
resPath = '../result/kmeans_demo/';
mkdir(resPath);
% k-means parameter
segNum = 10;
for i = 1:imgNum
    img = imread([dataPath, int2str(i), postfix]);
    src = imread([srcPath, 'input', int2str(i), '.jpg']);
